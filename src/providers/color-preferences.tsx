...
export const useColorPrefrences = () => {
  ...
  if (!context) {
    throw new Error('useColorPrefrences must be used within a ColorPrefrencesProvider');
  }

  return context;
};

export const ColorPrefrencesProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  return (
    <ColorPrefrencesContext.Provider value={value}>{children}</ColorPrefrencesContext.Provider>
  );
};
